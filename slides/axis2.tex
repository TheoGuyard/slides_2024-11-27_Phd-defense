...
\begin{frame}{Axis 2 -- Convex optimization}
  \begin{tikzpicture}[remember picture,overlay]
    \begin{scope}[xshift=0.5\linewidth]
      ...
      %
      \onslide<5-> {
        \node[text width=0.4\linewidth,align=center,anchor=north] (method) at ($(problem)+(-3,-1.25)$) {\textbf{First-order methods} \\ Proximal gradient \\ Coordinate descent \\ ...};
        %
  ...
  \begin{tikzpicture}[remember picture,overlay]
    \begin{scope}[xshift=0.5\linewidth]
      \onslide<1-> {
        ...
        $) (question) {\textbf{Task}};
        \node[draw,ultra thick,text width=0.45\linewidth, align=center] at ($(question.south)+(0,-0.25)$) (question1-text) {Reduce the solving cost};
      }
      %
      %
      %
      ...
      %
      \onslide<4-> {
        \node[text width=\linewidth,align=center,anchor=north] (scr) at ($(opt1)+(0,-1)$) {\textbf{Screening tests} \\ Identify zeros in $\opt{\pv}$ \\ \small{L. El Ghaoui \textit{et al.} (2011)}};
...
\begin{frame}{Axis 2 -- Screening and smoothing tests}
  \begin{tikzpicture}[remember picture,overlay]
    \begin{scope}[xshift=0.5\textwidth]
      \onslide<1-> {
        \node[text width=0.35\linewidth, align=center] (problem) at (-3.25,3.25) {
          \begin{blockcolor}{mDarkTeal}{Convex problem}
            \centering
            $\min_{\pv \in \kR^{\pdim}} \lfunc(\dic\pv) + \relaxrfunc(\pv)$
          \end{blockcolor}
      ...
      %
      \onslide<2-> {
        \node[align=center,text width=0.35\textwidth] (dual) at (3.25,3.25) {
      ...
      \onslide<5-> {
        ...
        %
        \draw[dashed,ultra thick,<->] (safe) -- ($(safe)+(0,0.8)$);
      }
      %
      %
      %
      \onslide<6-> {
        ...
        \node[draw,ultra thick,mLightBrown] at ($(spotlight.south)+(0,-0.25)$) (result-text) {Zeros and non-zeros in $\opt{\pv}$ can be identified from a safe region $\saferegion$.};
        %
        \node[text width=0.93\linewidth,align=left] (theorem) at (0,-2.75) {
          \textbf{Theorem --} Let $\saferegion$ be a safe region, one has
          \begin{alignat*}{6}
            \text{Screening test:} &&\quad \forall \dv \in \saferegion, \ \transp{\dici{\idxentry}}\dv &\in \interior(\subdiff\separable{\relaxrfunc}{\idxentry}(0)) &&\implies \opt{\pvi{\idxentry}} = 0 \\
            \text{Smoothing test:} &&\quad \forall \dv \in \saferegion, \  \transp{\dici{\idxentry}}\dv &\notin \subdiff\separable{\relaxrfunc}{\idxentry}(0) &&\implies \opt{\pvi{\idxentry}} \neq 0
          \end{alignat*}
        };
      }
      %
      ...
      %
      \onslide<7-> {
        \draw[ultra thick,mLightBrown] ($(theorem)+(-1.7,-0.8)$) -- ($(theorem)+(-1.7,-1)$) -- ($(theorem)+(2.4,-1)$) -- ($(theorem)+(2.4,-0.8)$);
...
\begin{frame}{Axis 2 -- Dynamic identification}
  \begin{tikzpicture}[remember picture,overlay]
    \begin{scope}[xshift=0.5\linewidth]
      ...
      %
      \onslide<5-> {
        \node[font=\small] (action-zero) at ($(identif-zero)+(0,-1)$) {Fix $\pvi{\idxentry} = 0$};
      ...
      %
      \onslide<7-> {
        \node[font=\small] (action-nzero) at ($(identif-nzero)+(0,-1)$) {Smooth $\separable{\relaxrfunc}{\idxentry}$};
      ...
      %
      \onslide<10-> {
        \node[font=\small,text width=0.4\linewidth,align=center] at ($(identif-nzero)+(0,-4)$) {Break non-smoothness \\ Higher-order iteration};
      }
      %
      %
      %
      ...
      \onslide<11-> {
        ...
        \draw[ultra thick,->,dashed] ($(safe-action)+(-1.25,2)$) .. controls ($(safe-action)+(0,2)$) .. (safe-action) node[midway,font=\small,xshift=10,rotate=-45] {mild cond.};
      }
    \end{scope}
  \end{tikzpicture}
\end{frame}
...
\begin{frame}{Axis 2 -- Numerics}
  \begin{tikzpicture}[remember picture,overlay]
    ...
    \begin{scope}[xshift=0.1\linewidth,yshift=-0.2\textheight]
      \onslide<2-> {
        \node[font=\small,text width=0.5\linewidth,align=left] at (6.75,1.5) {\textbf{Proximal gradient} \\ \ref{plot:ProximalGradient} Vanilla method \\ \ref{plot:ProximalGradientScr} With screening \\ \ref{plot:ProximalGradientScrRlx} With screening and smoothing};
      }
      %
      %
      %
      \onslide<4-> {
        \node[font=\small,rotate=-15] at (3,1.17) {linear};
        \node[font=\small,rotate=-25] at (3,0.45) {linear};
      }
      %
      %
      %
      \onslide<6-> {
        \node[font=\small,rotate=-87] at (0.5,0.75) {quadratic};
      }
      %
      %
      %
