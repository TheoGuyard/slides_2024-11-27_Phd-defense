  ...
  \begin{tikzpicture}[remember picture,overlay]
    \begin{scope}[xshift=0.5\textwidth]
      ...
      \onslide<3-> {
        ...
        %
        \draw[ultra thick,->] (problem) .. controls ($(problem)+(0,-1.75)$) .. (optcond) node[midway,fill=TolLightWhite,draw,ultra thick,font=\small] {solution $\opt{\pv}$};
        %
        \draw[ultra thick,->] (dual) .. controls ($(dual)+(0,-1.75)$) .. (optcond) node[midway,fill=TolLightWhite,draw,ultra thick,font=\small] {solution $\opt{\dv}$};
        %
      ...
      %
      \onslide<6-> {
        \node[TolLightOrange] (spotlight) at (0,-0.75) {\textbf{Spotlight result}};
        \node[draw,ultra thick,TolLightOrange] at ($(spotlight.south)+(0,-0.25)$) (result-text) {Some zeros and non-zeros in $\opt{\pv}$ can be identified from a safe region $\saferegion$.};
      }
      %
      %
      %
  ...
  \begin{tikzpicture}[remember picture,overlay]
    \begin{scope}[xshift=0.5\linewidth]
      ...
      \onslide<9-> {
        ...
        %
        \node[font=\small] (enet-smooth) at ($(enet)+(0,-1)$) {$\separable{\relaxrfunc}{\idxentry}(\pvi{}) = \pm\pvi{} + \pvi{}^2$};
        %
...
\begin{frame}{Axis 2 -- Numerics}
  \begin{tikzpicture}[remember picture,overlay]
    ...
    \begin{scope}[xshift=0.1\linewidth,yshift=-0.2\textheight]
      \onslide<2-> {
        \node[font=\small,text width=0.5\linewidth,align=left] at (7.5,1.5) {\textbf{Accelerated proximal gradient} \\ \ref{plot:ProximalGradient} Vanilla method \\ \ref{plot:ProximalGradientScr} With screening \\ \ref{plot:ProximalGradientScrRlx} With screening and smoothing};
      }
      %
      %
      %
      \onslide<4-> {
        \node[font=\small,rotate=-6] at (3,1.77) {linear};
