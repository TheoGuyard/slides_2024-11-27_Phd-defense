  ...
  \begin{tikzpicture}[remember picture,overlay]
    \begin{scope}[xshift=0.5\linewidth]
      ...
      \onslide<2-> {
        ...
        \begin{equation*}
          \textcolor{mLightBrown}{\rfunc_{\text{cvx}}}(\pvi{}) = 
          \begin{cases}
      ...
      \onslide<3-> {
        \begin{scope}[xshift=0.375\linewidth,yshift=-0.05\textheight,scale=0.9]
          ...
          %
          \onslide<6-> {
            \draw[ultra thick,mLightBrown,dashed] (-1,0.95) -- (0,0);
            \draw[ultra thick,mLightBrown,dashed] (0,0) -- (1,0.95);
            \draw[domain=1:1.75,smooth,variable=\x,mLightBrown,dashed,ultra thick] plot ({\x}, {0.45 + 0.5*\x*\x});
            \draw[domain=-1.75:-1,smooth,variable=\x,mLightBrown,dashed,ultra thick] plot ({\x}, {0.45 + 0.5*\x*\x});
            \fill[LavenderBlush4] (0,0) circle (0.1);
            \fill[mDarkTeal] (1,0.95) circle (0.1);
            \node[right,mLightBrown] at (-1.75,0.75) {$\rfunc_{\text{cvx}}$};
          }
        \end{scope}
      }
      %
...
\begin{frame}{Axis 1 -- Numerics}
  \begin{tikzpicture}[remember picture,overlay]
    \begin{scope}[xshift=0.5\linewidth]
      ...
      %
      \onslide<3-> {
        \node[text width=0.3\linewidth,align=center] (obs1) at (4,-1) {El0ps a \textbf{generic} BnB solver with \textbf{state-of-the-art} performance};
      }
    \end{scope}
    %
    %
