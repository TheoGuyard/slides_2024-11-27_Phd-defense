\documentclass[xcolor={x11names},10pt]{beamer}

\usetheme{metropolis}
\metroset{numbering=fraction}
\metroset{block=fill}

\newcommand{\backupbegin}{
  \newcounter{finalframe}
  \setcounter{finalframe}{\value{framenumber}}
}
\newcommand{\backupend}{
  \setcounter{framenumber}{\value{finalframe}}
}

\usepackage{headings/kmath}
\input{headings/packages}
\input{headings/macros}

\newenvironment<>{blockcolor}[2]{%
  \setbeamercolor{block title}{fg=#1,bg=#1!15}%
  \setbeamercolor{block body}{fg=#1,bg=#1!15}%
  \begin{block}#3{\centering#2}
}{%
  \end{block}
}

\definecolor{mLightWhite}{HTML}{FAFAFA}
\definecolor{mLightRed}{HTML}{CC6677}

\begin{document}

\begin{frame}[plain,noframenumbering]
  \begin{center}
    {\large\textbf{Branch-and-Bound algorithms \\ for $\boldsymbol{\ell}_{\boldsymbol{0}}$-regularized problems}}
  \end{center}
  \begin{center}
    Theo Guyard \\
    {\small{Inria and Insa Rennes}} \\
    {\small{PhD defense - November 27th, 2024}}
  \end{center}
\end{frame}

\input{slides/intro}
\input{slides/bnb}
\input{slides/axis1}
\input{slides/axis2}
\input{slides/axis3}
\input{slides/conclusion}

\begin{frame}[standout]
  Question time ! ~\\~\\
  \includegraphics[width=40pt]{img/nerd-face.png}
\end{frame}

\backupbegin
\begin{frame}
  \AddTodo{
    \begin{itemize}
      \item Slide peeling
      \item Slide complexity iter / time screen \& relax
      \item Slide El0ps
      \item Slide proximal identification / screening
      \item Slide formulation MIP
      \item Slide relaxation avec $\nodeSymb$ dans convex
      \item Slide problème lissé screen / relax
      \item Simultaneous pruning avec tout les indices
    \end{itemize}
  }
\end{frame}
% \input{slides/supplements}
\backupend


\end{document}
